...
RF_mtr = mtrHAT.getMotor( rf_mtr_ch )     # RF_mtr = right front motor 
...
def releaseAllMotors(): 
	mtrHAT.getMotor(1).run( Adafruit_MotorHAT.Adafruit_MotorHAT.RELEASE ) 
	mtrHAT.getMotor(2).run( Adafruit_MotorHAT.Adafruit_MotorHAT.RELEASE ) 
	mtrHAT.getMotor(3).run( Adafruit_MotorHAT.Adafruit_MotorHAT.RELEASE ) 
	mtrHAT.getMotor(4).run( Adafruit_MotorHAT.Adafruit_MotorHAT.RELEASE ) 
...
def goTurnR(speed): 
	...
	RF_mtr.run( FORWARD ) 
	LR_mtr.setSpeed( speed ) 
	RR_mtr.setSpeed( speed ) 
	LF_mtr.setSpeed( speed ) 
	RF_mtr.setSpeed( speed ) 
